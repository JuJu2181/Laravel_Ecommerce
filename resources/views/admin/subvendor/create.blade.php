@section('title',Auth::user()->name.' - Create New Sub-Vendor')
<x-admin.layout>
<div class="az-content az-content-dashboard">
    <div class="container">
        @unless(Auth::user()->role == 'user' || Auth::user()->vendor_staus == 'not_verified')
        <div class="az-content-body">
            <h2>Create New Sub Vendor for <span id="vendor_name">{{Auth::user()->name}}</span></h2>
            {{-- check if user is authorized to create a product --}}
            <form action={{ route('admin.subvendors.store') }} method="post" enctype="multipart/form-data">
                @csrf
                {{-- for name --}}
                <label for="name" class="m-1">Sub-Vendor Name: </label> 
                <input type="text" name="name" id="name" class="form-control @error('name') is-invalid @enderror" placeholder="sub-vendor name">
                @error('name')
                ...
                value="email will be autogenerated based on name">
                @error('email')
                <div class="alert alert-danger mt-2">{{ $message }}</div>
                @enderror
                {{-- password --}}
                ...
                <label class="mr-3"><input type="checkbox" name="responsibility[]" value="{{strtolower($responsibility)}}"> {{$responsibility}} 
                </label>
                @endforeach
                </div>
                ...
                @enderror
                {{-- submit btn --}}
                <input type="submit" value="Create Sub vendor" name="submit" class="btn btn-success btn-block mt-4">
            </form>
        </div>
        @else 
